...
def V(t):
    """
    Function to return the voltage at time t in ms
    :param t: time instance (or vertor) in ms
    :return: voltage value in mV at a given time instance
    """
    return volts_interpolated(t/ 1000)

def V_torch(t):
    """
    Function to return the voltage at time t in ms as a tensor
    :param t: time instance (or vertor) in ms
    :return: voltage value in mV at a given time instance as a tensor
    """
    return pt.tensor(volts_interpolated(t/ 1000))

def collocm(splinelist, tau):
    """
    Function to compute the collocation matrix for B-spline values (0-derivative)
    :param splinelist: list of splines along one axis,
    :param tau: - interval on which we wish to evaluate splines
...
protocolFolder = direcory_names["input_data"]
# #  load the voltage data:
# volts = np.genfromtxt(protocolFolder+"/protocol-staircaseramp.csv", skip_header=1, dtype=float, delimiter=',')
# #  check when the voltage jumps
# read the times and valued of voltage clamp
volt_times, volts = np.genfromtxt(protocolFolder+"/protocol-staircaseramp.csv", skip_header=1, dtype=float, delimiter=',').T
...
times_ap, voltage_ap = np.genfromtxt(protocolFolder+'/ap.csv', delimiter=',', skip_header=1).T
...
###################################################################################################################
# these can be global as thet are features of a specific protocol we use for training
V_min = -120
V_max = 60
tlim = [0, 14899]
...
if __name__ == '__main__':
    ...
    plt.tight_layout()
    plt.savefig(figureFolder+'/staircase_protocol.png')
    ...
    ax = pretty_axis(ax, legendFlag=False)
    plt.tight_layout()
    plt.savefig(figureFolder+'/ap_protocol.png')