...
# imports
from methods.load_protocols import *
import methods.load_protocols as load_protocols

# defnitions
def hh_model(t, x, theta):
    """
    This function computes the ODE for the HH model with parameters given in theta
    :param t: time for which the ODE is solved
    :param x: gating variable vector state
    :param theta: vector of model parameters
    :return:
    vector of derivatives of the gating variables
    """
    a, r = x[:2]
    *p, g = theta[:9]
    v = V(t)
...
def kemp_model(t, x, theta):
    """
    This function computes the ODE for the Kemp model with parameters given in theta
...
def kemp_model_ss(t, x, theta, v):
    ...
    :param x: gating variable vector state
    :param theta: vector of model parameters
    :param v: voltage at which the steady state is computed
    :return:
    vector of derivatives of the gating variables
    """
    op, c1, h = x[:3]
...
def wang_model(t, x, theta):
    """
    This function computes the ODE for the Wang model with parameters given in theta
    C3 <- (b_a0, a_a0) -> C2 <-(k_b, k_f)-> C1 <-(b_a1, a_a1)-> O <-(b_1, a_1)-> I
    activation: 4 state MM
    inactivation: 2 state MM
    :param t: time for which the ODE is solved
    :param x: the gaiting variable vector state
    :param theta: vector of model parameters
    :return:
    vector of derivatives of the gating variables
    """
...
def wang_model_ss(t, x, theta, v):
    """
    This function computes the steady state of the Wang model at voltage v
    ...
    :param v: voltage at which the steady state is computed
    :return:
    the derivatives of the gating variables at the steady state
    """
    op, c1, c2, c3, i_h = x[:5]
    *p, g = theta[:15]
    a_a0 = p[0]*np.exp(p[1] * v)
...
def observation_kemp(t, ode_solution, theta, snr=None):
    """
    This function computes the Kemp current from the ODE solution and the voltage passed as time series
    :param t: time for which the observation is computed
    :param ode_solution: ode solution object for all gating variables
    :param theta: vector of model parameters
    :param snr: signal to noise ratio
    :return:
    noiseless_output: the observation computed from the ODE solution without any noise
    noisy_output: the observation computed from the ODE solution with noise
    """
...
def observation_wang(t, ode_solution, theta, snr=None):
    """
    This function computes the Wang current from the ODE solution and the voltage passed as time series
    :param t: time for which the observation is computed
    :param ode_solution: ode solution object for all gating variables
    :param theta: vector of model parameters
    :param snr: signal to noise ratio
...
def observation_hh(t, ode_solution, theta, snr=None):
    """
    This function computes the HH current from the ODE solution and the voltage passed as time series
    :param t: time for which the observation is computed
    :param ode_solution: ode solution object for all gating variables
    :param theta: vector of model parameters
    :param snr: signal to noise ratio
...
def observation(t, x, theta):
    ...
    :param t:  time for which the observation is computed
    :param x:  the solution of the ODE - gating variable states
    :param theta: vector of model parameters
    :return:
    noiseless_output: the observation computed from the ODE solution without any noise
    """
...
def observation_direct_input(x, v, theta):
    ...
    This function computes the observation from the ODE solution and the voltage passed as time series
    :param x: the solution of the ODE - gating variable states
    :param v: the voltage time series
    :param theta: vector of model parameters
    :return:
    noiseless_output: the observation computed from the ODE solution without any noise
    """
...
def observation_log(t, ode_solution, theta_log):
    """
    This function computes the observation from the approximate ODE solution as array of states but the conductance
    is given in log space
    :param t: time for which the observation is computed
    :param ode_solution: the solution of the ODE - gating variable states
    :param theta_log: the parameters of the model in log space
    :return:
    noiseless_output: the observation computed from the ODE solution without any noise
    """
    x = ode_solution
    *ps_log, g_log = theta_log[:9]
    # generate noise from normal distr to match dimension of the output
...
def generate_synthetic_data(model_name, thetas_true, times):
    ...
    :param model_name: the model name chosed for synthetic data generation
    :param thetas_true: the true model parameters (vector)
    :param times: the time vector for which the synthetic data will be generated
    :return:
    solution: the solution of the ODE for the model specified by model_name
    current_model: the function that computes the observation from the ODE solution, according to model name
    """
...
def split_generated_data_into_segments(solution, modelled_current, jump_indeces, times):
    """
    This function splits the generated data into segments based on the jumps in the protocol
    :param solution: the solution of the ODE for the model specified by model_name
    :param modelled_current: the noisy current measurement that is produced by the measurement model
    :param jump_indeces: indeces of jumps in the staircase protocol
    :param times: the time vector for which the synthetic data was generated
    :return:
    states_roi: list of states for each segment
    states_known_roi: list of known states for each segment
    current_roi: list of current for each segment
    """
    states_roi = []
    states_known_roi = []
    current_roi = []
...
if __name__ == '__main__':
    ...
    for iAx, ax in enumerate(axes):
        ...
        ax.legend(loc='best')
    plt.tight_layout()
    fig.savefig(figureFolder + '/current_true_test_model_'+model_name+'.png', dpi=400)
    ...
    for ax in axes:
        ...
        ax = pretty_axis(ax)
    plt.tight_layout()
    fig.savefig(figureFolder + '/generated_data_test_model_'+model_name+'.png',dpi=400)
    # test complete
