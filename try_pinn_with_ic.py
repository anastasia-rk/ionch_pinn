...
lambda_l1 = 1e-2 # weight on the L1 norm of the parameters
...
if rhs_name == 'test':
    ...
    time_of_domain = domain.detach().numpy()[:, 0]
...
for i in tqdm(range(250001)):
    ...
    if i % 10000 == 0:
        ...
        print("Cost gradient: " + str(store_grad))
        ...
        if rhs_name == 'test':
            axes[1].set_ylim(-2, 1)
        fig.tight_layout()
        fig.savefig('Figures/'+rhs_name+'_NN_approximation_iter_' + str(i) + '.png')
        if i % 20000 == 0:
            plt.show()
...
plt.tight_layout()
plt.savefig('Figures/Example_with_disturbance.png')
plt.show()