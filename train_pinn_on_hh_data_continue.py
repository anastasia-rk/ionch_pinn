...
if __name__ == '__main__':
    ...
    nSamples = 20 # number of samples in the training dataset
    nPerBatch = 60 # number of samples per batch
    ...
    for i in tqdm(range(start=initIter + maxIter, stop=initIter + 1)):
        ...
        for i_batch, (input_batch, *precomputed_RHS_batch, target_batch) in enumerate(dataloader):
            ...
            running_losses = [running_IC_loss, running_RHS_loss, running_L1_loss, running_data_loss,
        ...
        if i % plotEvery == 0:
            ...
            dxdt, rhs_pinn, current_pinn = compute_derivs_and_current(stacked_domain, state_domain,
                                                                      precomputed_RHS_params, scaling_coeffs,
                                                                      device)
            ################################################################################################################
            ...
            axes[nOutputs].plot(unique_times, measured_current, label="Measured current", color="k", linewidth=1,
                                alpha=0.3)
            # for as many conductances as we put in, plot the current
            for iSample in range(0, nSamples):
                # plot the current
                axes[nOutputs].plot(unique_times, current_pinn[iSample, :].cpu().detach().numpy(),
                                    color=colours[iSample], linewidth=0.5, alpha=0.7)  # label = "PINN current"
    ...
    state_true_all = pinn_state.cpu().detach().numpy()
    for iOutput in range(nOutputs):
        ...
        for iSample in range(0, nSamples):
            axes[iOutput].plot(times, pinn_output[iSample, :, iOutput], '--', color=colours[iSample], alpha=0.7,
                               linewidth=0.5)
