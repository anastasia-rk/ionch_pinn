...
if __name__ == '__main__':
    ...
    for iOutput in range(nOutputs):
        axes[iOutput].plot(unique_times, state_true_all[iOutput,:], label='IVP solution', color='k', alpha=0.3)
    ...
    if model_name.lower() == 'hh':
        ...
        axes = axes.ravel()
        for iOutput in range(nOutputs):
            axes[iOutput].plot(unique_times, state_true_all[iOutput,:], label='True state',color='k', alpha=0.3)
        ...
        iAxis = nOutputs
        # plot current vs PINN current
        axes[iAxis].plot(unique_times, measured_current, label='True current',color='k', alpha=0.3)
