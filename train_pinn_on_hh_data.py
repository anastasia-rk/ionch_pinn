...
if __name__ == '__main__':
    ...
    # note that dataloader (at least the one running on HPC) does permit tensors that require grad to be sent to it
    dataset = TensorDataset(stacked_domain, *precomputed_RHS_params,  measured_current_tensor)
    # make stacked_domain required grad after we sent it to the dataloader
    stacked_domain_grad = stacked_domain.requires_grad_(True)
    ...
    for i in tqdm(range(maxIter)):
        ...
        for i_batch, (input_batch, *precomputed_RHS_batch, target_batch) in enumerate(dataloader):
            # require grad for the input batch and target batch
        ...
        if i % plotEvery == 0:
            ...
            state_domain = pinn(stacked_domain_grad)
            # use custom detivarive function to compute the derivatives of outputs, because grad assumed that the output is a scalar
            dxdt = derivative_multi_input(state_domain, stacked_domain_grad)
            dxdt = dxdt * t_scaling_coeff # restore to the original scale
            rhs_pinn = RHS_from_precomputed(state_domain, precomputed_RHS_params)
            current_pinn = observation_tensors(time_of_domain, state_domain, stacked_domain_grad)
            current_pinn = current_pinn/param_scaling_coeff
            ################################################################################################################
